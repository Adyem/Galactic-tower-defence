...
- `[`/`]`: lower/raise master volume; accessibility and volume preferences are saved in the profile.
- `R`: restart with the selected loadout; `Esc`: quit.

The window title reports weapon, wave, score, and state. The renderer draws a logical 1280×720 canvas and falls back to SDL’s software renderer when accelerated rendering is unavailable, which is useful for CI and remote machines.
...
- SDL2 window/input/rendering, responsive logical scaling, pooled-style vector collections, and a no-font dependency visual HUD.
...
- Versioned profile and replay persistence with atomic writes (`tower_ascend.profile` and `tower_ascend.last.replay` are created after a completed run).
