...
./build/tower_ascend --render-smoke --text-audit
...
The bitmap renderer has a diagnostic mode that measures every label against the
1280x720 viewport and checks bounded descriptions against their declared boxes.
It reports each unique overflow to stderr and to `text_layout.log`. Use
`--text-audit` during a render smoke pass or set `TA_TEXT_AUDIT=1`. The audit is
disabled during normal play, and new bounded text should use the shared
`drawWrappedTextInBox` helper so its width and height are checked automatically.
...
- During an upgrade draft, `R` spends the run's single reroll; outside a draft it retains its restart behavior.
- During a run, `1`–`5` select the five active skills; targeted skills enter a placement/area preview, confirm with a world click, and cancel with `Esc`/right-click. The separate bottom-right ultimate remains manually activated with `Space`/`U`.
- The Workshop exposes generic-to-specialist skill trees. Click an equipped skill tree to purchase the next legal node, use `6`–`9` to unlock the four initially locked skills, and click skill slots in Loadout to cycle unlocked skills without duplicates.
...
Roadmap systems currently included in this build:

- Authored Daily missions with distinct briefings, required chassis/weapons, enemy rosters, skulls, objectives, rewards, and enforced setup rules.
- Profile-backed Workshop progression, Legend Cores, mastery-gated Ultimate Evolutions, Tower Chassis, and Support Modules.
- Ten authored active skills covering gravity control, displacement traps, allied squads, temporary barracks, weakness, healing/buffs, deployable towers, cryo control, drones, and a data-only authored-operation fixture; each has cooldowns/charges, deterministic targeting, runtime entities, replay events, and a five-slot loadout.
- Authored skill talent trees begin with generic radius/cooldown/charge nodes and branch into behavior-changing specializations. Talent purchases, equipped builds, and three named skill presets persist in profile schema 15, while Daily challenges loan a distinct required skill for each date/theme.
- A Codex covering weapons, upgrades, enemies, synergies, ultimates, support, chassis, skulls, arenas, and evolutions.
- Results reward explanations, measured damage/reaction/status output, threat-specific failure guidance, next-wave previews, and one deterministic upgrade reroll.
- Six balance policies across fresh/mid/max profiles with damage, reaction, status, kill, leak, score, and wave telemetry.
- Balance output also reports each Oracle weapon/arena cell and every authored Legendary Evolution and ultimate sidegrade; use `ta_balance_check 100` for a 100-seed audit distribution per matrix combination.
...
- Offline daily challenge rotation with a date-derived seed, themed title/description, recommended weapon, skull recipe, arena, enemy pressure, objective, and first-clear reward preview.
...
- Persistent progression: runs award Core Parts, Daily first clears award protected Legend Cores, five tower skins can be unlocked/equipped with cosmetic Shards, and Workshop node purchases are profile-backed with bounded levels.
- Main Menu, Run Type, Loadout, Workshop, Collection, and Settings frontend states are available; loadout weapon, skull, and ultimate cards expose hover descriptions.
- Loadout shows saved currencies, daily briefing information, and best score before each run.
...
- Ultimate module sidegrades are authored in `assets/content/ultimate_modules.json`, require mastery and Core Parts, persist in profile schema 15, and are replay/state-hash deterministic.
...
- `ta_balance_check` runs all 16 skull combinations across the weapon × arena matrix using first-choice, random, tag-match, and synergy decision policies, reporting victory/terminal rates and average scores.

The next production steps are expanding the front-end flow, workshop progression, descriptive UI, build synergies, content manifests, production art/audio assets, accessibility/localization, and platform packaging. See [`LUNA_IMPLEMENTATION_ROADMAP.md`](LUNA_IMPLEMENTATION_ROADMAP.md) for the implementation-ready menu, workshop, tooltip, progression, and strategic-balance roadmap. The pure simulation in `src/game.*` is deliberately isolated so those additions do not require rewriting combat.
